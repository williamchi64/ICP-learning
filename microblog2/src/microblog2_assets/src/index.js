import {
  microblog2, createActor
} from "../../declarations/microblog2";
import {
  Principal
...
async function set_author() {
  ...
  try {
    await microblog2.set_name(author_text.value);
...
async function post() {
  ...
  try {
    await microblog2.post(password.value, message.value);
    message.value = "";
  } catch (err) {
    console.log(err);
...
async function follow() {
  ...
  try {
    ...
    await microblog2.follow(follow_principal);
    follow_text.value = "";
  } catch (err) {
    console.log(err);
...
async function load_follows() {
  ...
  let follows = await microblog2.follows();
...
async function load_timeline() {
  ...
  let timeline = await microblog2.timeline(index.filter_time);
